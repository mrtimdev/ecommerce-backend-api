  ...
  <DefaultLayout>
    ...
    <div class="container">
      ...
      <div class="content-body p-5">
        <div class="relative">
          <table id="steerings" class="table w-full text-sm text-left rtl:text-right">
            <thead class="text-center bg-white dark:bg-boxdark">
              <tr>
                ...
                <th class="w-[20%]">{{ $t("code") }}</th>
                <th class="w-[30%]">{{ $t("name") }}</th>
                <th class="w-[37%]">{{ $t("description") }}</th>
...
const openModalForm = () => {
  events.emit('modal:open', {
    modal_title: "steering.add",
    event_type: "add",
  });
}
const breadcrumbs = reactive([
  { label: "Home", url: route("dashboard") },
  { label: t("steerings"), url: null },
...
onMounted(() => {
  tableData.value = $("#steerings").DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [[1, "desc"]],
