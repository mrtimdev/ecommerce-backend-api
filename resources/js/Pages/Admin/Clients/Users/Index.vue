<template>
  <LayoutTab>
    <Head :title="$t('clients_list')" />
    <div class="container">
      <div
        class="content-header rounded-tl-md rounded-tr-md py-4 border-b bg-white dark:border-gray-700 dark:bg-boxdark-1 flex flex-grow items-center justify-between"
      >
      ...
      <div class="content-body">
        ...
        >
          ...
          <div>You can control your clients account here.</div>
        </div>
        <div class="relative">
          <table id="clients" class="table w-full text-sm text-left rtl:text-right">
            <thead class="text-center bg-white dark:bg-boxdark">
              <tr>
                ...
                <th class="w-[15%]">{{ $t("created_by_owner") }}</th>
...
onMounted(() => {
  tableData.value = $("#clients").DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [
                    ...
                    default: () => [
                      h(
                        "div",
                        {
                          ...
                          onClick: (e) => {
                            router.get(route("clients.edit", row.id));
                          },
                        },
...
events.on("confirm:confirmed", (data) => {
  ...
  form.ids = items;
  form.post(route("clients.destroy.selected"), {
    preserveScroll: true,
    onSuccess: () => {
      const Toast = proxy.$swal.mixin({
        toast: true,
