  ...
  <LayoutTab>
    ...
    <div class="container">
      ...
      >
        ...
        >
          <i class="fi fi-rr-plus w-4 h-4"></i> {{ $t("client.add") }}
        </div>
      </div>
              ...
              >
                <div>
                  <Label
                    ...
                    >{{ $t("avatar") }}</Label
                  >
                  <FileUpload v-model="form.avatar" target_input="avatar" />
  ...
  form.post(route("clients.store"), {
    ...
    onSuccess: (res) => {
      events.emit("toaster", {
        type: "success",
        action: "create",
        message: `${t("client")} [${form.first_name} ${form.last_name}] ${t(
          "successfully_added"
        )}`,
      });
    },
