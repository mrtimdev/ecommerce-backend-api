...
<script setup>
  ...
  const name = ref(null)
  const modal_title = ref('driveType.add')
  const event_type = ref('add')
  events.on('modal:open', (data) => {
    modal_title.value = data.modal_title || 'driveType.add'
  ...
  const submitForm = () => {
    const routeName = event_type.value === 'edit' ? 'driveTypes.update' : 'driveTypes.store'
  ...
  events.on('modal:success', () => {
    isVisible.value = true
    form.reset("id", "code", "name", "is_active")
    if(event_type.value === 'edit') {
      isVisible.value = false
    }
    form.clearErrors() 
  ...
  events.on('delete-items', (ids) => {
    var routeName = route('driveTypes.destroy.selected', {
                    ids: ids
                  })
    form.post(routeName, {
      preserveScroll: true,
