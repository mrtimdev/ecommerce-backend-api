<template>
  <DefaultLayout>
    <Head :title="$t('transmission_types')" />
    <div class="container">
      <div
      ...
      <div class="content-body p-5">
        <div class="relative">
          <table id="transmission_types" class="table w-full text-sm text-left rtl:text-right">
...
const openModalForm = () => {
  events.emit('modal:open', {
    modal_title: "transmission_type.add",
    event_type: "add",
  });
}
const breadcrumbs = reactive([
  { label: "Home", url: route("dashboard") },
  { label: t("transmission_types"), url: null },
...
onMounted(() => {
  tableData.value = $("#transmission_types").DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [[1, "desc"]],
    ...
    ajax: {
      url: route("transmissionTypes.list"),
      type: "GET",
    },
    columns: [
      {
                      ...
                      h(
                        "div",
                        {
                          ...
                          onClick: (e) => {
                            e.preventDefault();
                            events.emit('modal:open', {
                              modal_title: "transmission_type.edit",
                              event_type: "edit",
                              item: row
                            });
                          },
...
events.on('confirm:confirmed', (data) => {
  ...
  transmissionTypeStore.deleteTransmissionTypes(items);
})
