<template>
    <Teleport to="body">
        <modal size="md" :show=" isVisible " :show-footer=" false " :show-confirm-button=" true " button-confirm-label="save" @close="closeModal">
          ...
          <template #body>
            <form @submit.prevent="transmissionTypeStore.submitForm">
              <div class="p-4 md:p-5">
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                  <div>
                    <label for="code" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">{{
                      $t('code') }}</label>
                    <input type="text" ref="code" id="code" v-model=" transmissionTypeStore.form.code "
                      ...
                      :placeholder=" $t('code') " />
                    <InputError :message=" transmissionTypeStore.form.errors.code " class="mt-2" />
                  </div>
                  <div>
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">{{
                      $t('name') }}</label>
                    <input type="text" ref="name" id="name" v-model=" transmissionTypeStore.form.name "
                      class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-boxdark-1 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 dark:bg-tim-11"
                      :placeholder=" $t('name') " />
                    <InputError :message=" transmissionTypeStore.form.errors.name " class="mt-2" />
                  </div>
                ...
                <div class="mb-6">
                  ...
                  <select v-model=" transmissionTypeStore.form.is_active "
                    id="status"
                  ...
                  <InputError :message=" transmissionTypeStore.form.errors.is_active " class="mt-2" />
                </div>
...
<script setup>
  ...
  const { t } = useI18n();
  import {useTransmissionType} from "@/stores/transmissionType";
