  ...
  <DefaultLayout>
    ...
    <div class="container">
      ...
      <div class="content-body p-5">
        <div class="relative">
          <table id="passengers" class="table w-full text-sm text-left rtl:text-right">
            <thead class="text-center bg-white dark:bg-boxdark">
              <tr>
                ...
                <th class="w-[70%]">{{ $t("no_of_passenger") }}</th>
...
const openModalForm = () => {
  events.emit("modal:open", {
    modal_title: "passenger.add",
    event_type: "add",
  });
};
const breadcrumbs = reactive([
  { label: "Home", url: route("dashboard") },
  { label: t("passengers"), url: null },
  { label: t("list"), url: null, is_active: true },
]);

onMounted(() => {
  tableData.value = $("#passengers").DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [[1, "desc"]],
    ...
    columns: [
      ...
      { data: "code", name: "code" },
      {
        ...
        name: "no",
        render: function (data, type, row, meta) {
          return t("seat.count", { count: data });
        },
      },
      {
        data: "action",
