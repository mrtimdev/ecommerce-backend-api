            ...
            <form @submit.prevent="submitForm">
              <div class="p-4 md:p-5">
                <div class="grid gap-6 mb-6 grid-cols-1">
                  <div>
                    <label for="no" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">{{
                      $t('no') }}</label>
                    <input type="text" ref="no" id="no" v-model=" form.no "
...
<script setup>
  ...
  const submitForm = () => {
    const routeName = event_type.value === 'edit' ? 'passengers.update' : 'passengers.store'
