    ...
    <div class="">
      ...
      >
        <div class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          <h2 class="text-md font-medium text-gray-700 hover:text-purple-600 dark:text-gray-400 dark:hover:text-white">{{ $t('stories') }}</h2>
        </div>
        ...
        <table id="stories"
          ...
          <thead class="text-center bg-white dark:bg-boxdark">
            <tr>
              ...
              <th class="w-[30%]">{{ $t('country') }}</th>
              <th class="w-[30%]">{{ $t('youtube_link') }}</th>
...
onMounted(() => {
  ...
  tableData.value = $('#stories').DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [[1, 'desc']],
    ...
    ajax: {
      url: route('frontend.page.stories.list'),
      type: 'GET',
    },
    columns: [
      {
      ...
      { data: 'country', name: 'country',
        ...
        searchable: false,
        render: function (data, type, row, meta) {
          return countryFlagFormat(row.country, true);
        }
      },
      { data: 'youtube_link', name: 'youtube_link', 
        ...
        searchable: false,
        render: function (data, type, row, meta) {
          return extractYouTubeVideoId(data, true)
        }
      },
