<template>
  <LayoutTab>
    <Head :title="$t('frontend.services')" />
    <div class="">
      <div
      ...
      >
        <div class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          <h2 class="text-md font-medium text-gray-700 hover:text-purple-600 dark:text-gray-400 dark:hover:text-white">{{ $t('services') }}</h2>
        </div>
        ...
        <div class="mt-4">
          <div class="grid gap-6 mb-6 md:grid-cols-2">
            <fieldset class="border border-purple-300 dark:border-purple-400 p-4 rounded-lg">
              <legend class="block mb-2 text-sm font-medium !text-purple-600">{{ $t('service_info') }}</legend>
              <div>
                <div class="">
                  <form @submit.prevent=" serviceFormSubmit " enctype="multipart/form-data">
                    <div class="mb-6">
                      <Label for_id="title" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">
                        {{ $t('title') }}</Label>
                      <input type="text" id="title" ref="title" v-model=" serviceForm.title "
                        ...
                        :placeholder=" $t('title') " />
                      <InputError :message=" serviceForm.errors.title " class="mt-2" />
                    </div>
                    <div>
                      <Label  
                        for_id="image_path"
                        ...
                        >{{ $t("image") }}</Label
                      >
                      <FileUpload v-model="serviceForm.image_path" target_input="image_path" :selectedFile="selectedImage" />
                      <InputError :message="serviceForm.errors.image_path" class="mt-2" />            
                    </div>
            ...
            <fieldset class="border border-purple-300 dark:border-purple-400 p-4 rounded-lg">
              <legend class="block mb-2 text-sm font-medium !text-purple-600">{{ $t('service_items') }}</legend>
              <div >
                  <form @submit.prevent=" serviceItemsFormSubmit " enctype="multipart/form-data">
                    <div >
                      <div class="mb-6">
                        <Label for_id="item_title" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">
                          {{ $t('title') }}</Label>
                        <input type="text" id="item_title" ref="item_title" v-model=" serviceItemForm.title "
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-boxdark-1 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 dark:bg-tim-11"
                          :placeholder=" $t('title') " />
                        <InputError :message=" serviceItemForm.errors.title " class="mt-2" />
                      </div>
                      <div class="mb-[19px]">
                        <Label for_id="description" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">{{ $t('description') }}</Label>
                        <Textarea rows="8" id="description" v-model="serviceItemForm.description"></Textarea>
                        <InputError :message="serviceItemForm.errors.description" class="mt-2" />
                      </div>
                    </div>
...
selectedImage.value = props.service?.image_path


const serviceFormSubmit = () => {
  serviceForm.post(route('frontend.page.services.store'), {
    onSuccess: () => {
...
const openModalForm = () => {
  events.emit('modal:open', {
    modal_title: "service.add",
    event_type: "add",
  });
}

onBeforeUnmount(() => {
  serviceItem.value = false
})
onMounted(() => {

tableData.value = $('#serviceItems').DataTable({
  processing: true,
  serverSide: true,
  pageLength: 10,
  order: [[1, 'desc']],
  ...
  ajax: {
    url: route('frontend.page.serviceItems.list'),
    type: 'GET',
  },
  columns: [
    {
                    ...
                    h(
                      "div",
                      {
                        ...
                        onClick: (e) => {
                          ...
                          serviceItemForm.id = row.id
                          serviceItemForm.title = row.title
                          serviceItemForm.description = row.description ?? ""
                          serviceItemForm.clearErrors()
...
const cancelUpdateItem = () => {
  ...
  serviceItemForm.id = null
  serviceItemForm.title = ""
  serviceItemForm.description = ""
  serviceItemForm.clearErrors()
  item_title.value.focus()
}

const serviceItemsFormSubmit = () => {
  if(serviceItemForm.id) {
    serviceItemForm.post(route('frontend.page.serviceItems.update', serviceItemForm), {
      onSuccess: () => {
    ...
    serviceItemForm.post(route('frontend.page.serviceItems.store'), {
      onSuccess: () => {
        serviceItemForm.reset()
        item_title.value.focus()
        tableData.value.ajax.reload();
...
events.on('confirm:confirmed', (data) => {
  ...
  var routeName = route('frontend.page.serviceItems.destroy.selected', {
                    ids: items
                  })
  serviceItemForm.post(routeName, {
    preserveScroll: true,
      onSuccess: () => {
