    ...
    <div class="">
      ...
      >
        <div class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          <h2 class="text-md font-medium text-gray-700 hover:text-purple-600 dark:text-gray-400 dark:hover:text-white">{{ $t('videos') }}</h2>
        </div>
...
const { statusFormat, imageFormat, extractYouTubeVideoId } = useHelper()
...
onMounted(() => {
  ...
  tableData.value = $('#video').DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [[1, 'desc']],
    ...
    ajax: {
      url: route('frontend.page.videos.list'),
      type: 'GET',
    },
    columns: [
      {
