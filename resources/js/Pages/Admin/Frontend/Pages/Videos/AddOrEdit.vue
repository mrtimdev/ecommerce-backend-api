            ...
            <form @submit.prevent=" videoFormSubmit " enctype="multipart/form-data">
              <div class="p-4 md:p-5">
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                  <div>
                    <Label :isRequired="true" for_id="title" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">
                      {{ $t('title') }}</Label>
...
<script setup>
  ...
  const modal_title = ref('video.add')
  ...
  events.on('modal:open', (data) => {
    modal_title.value = data.modal_title || 'video.add'
  ...
  const videoFormSubmit = () => {
    if(event_type.value === "edit") {
      form.post(route('frontend.page.videos.update', form), {
        onSuccess: () => {
          nextTick(() => {
            form.reset()
            isVisible.value = false
      ...
      form.post(route('frontend.page.videos.store'), {
        onSuccess: () => {
          nextTick(() => {
            form.title = ''
            form.youtube_link = ''
