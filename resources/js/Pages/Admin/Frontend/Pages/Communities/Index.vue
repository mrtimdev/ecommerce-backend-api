    ...
    <div class="">
      ...
      >
        <div class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          <h2 class="text-md font-medium text-gray-700 hover:text-purple-600 dark:text-gray-400 dark:hover:text-white">{{ $t('communities') }}</h2>
        </div>
        ...
        <div class="mt-4">
          <div class="grid gap-6 mb-6 md:grid-cols-2">
            <fieldset class="border border-purple-300 dark:border-purple-400 p-4 rounded-lg">
              <legend class="block mb-2 text-sm font-medium !text-purple-600">{{ $t('community_info') }}</legend>
              <div>
                <div class="">
                  <form @submit.prevent=" communityFormSubmit " enctype="multipart/form-data">
                    <div class="mb-6">
                      <Label for_id="title" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">
                        {{ $t('title') }}</Label>
                      <input type="text" id="title" ref="title" v-model=" communityForm.title "
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                      <div>
                        <Label for_id="button_label" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">
                          {{ $t('button_label') }}</Label>
                        <input type="text" id="button_label" ref="button_label" v-model=" communityForm.button_label "
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-boxdark-1 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 dark:bg-tim-11"
                          :placeholder=" $t('button_label') " />
                        <InputError :message=" communityForm.errors.button_label " class="mt-2" />
                      </div>
                      <div>
                        <Label for_id="button_url" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">
                          {{ $t('button_url') }}</Label>
                        <input type="text" id="button_url" ref="button_url" v-model=" communityForm.button_url "
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-boxdark-1 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 dark:bg-tim-11"
                          :placeholder=" $t('button_url') " />
                        <InputError :message=" communityForm.errors.button_url " class="mt-2" />
                      </div>
                    </div>

                    <div class="mb-[19px]">
                      <Label for_id="description" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">{{ $t('description') }}</Label>
                      <Textarea rows="5" id="description" v-model="communityForm.description"></Textarea>
            ...
            <fieldset class="border border-purple-300 dark:border-purple-400 p-4 rounded-lg">
              <legend class="block mb-2 text-sm font-medium !text-purple-600">{{ $t('community_items') }}</legend>
              <div >
                  <form @submit.prevent=" communityItemsFormSubmit " enctype="multipart/form-data">
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                      <div>
                        <Label for_id="item_title" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">
                          {{ $t('title') }}</Label>
                        <input type="text" id="item_title" ref="item_title" v-model=" communityItemForm.title "
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-boxdark-1 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 dark:bg-tim-11"
                          :placeholder=" $t('title') " />
                        <InputError :message=" communityItemForm.errors.title " class="mt-2" />
                      </div>
                      <div>
                        <Label for_id="item_link" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">
                          {{ $t('link') }}</Label>
                        <input type="text" id="item_link" ref="item_link" v-model=" communityItemForm.link "
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-boxdark-1 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 dark:bg-tim-11"
                          :placeholder=" $t('link') " />
                        <InputError :message=" communityItemForm.errors.link " class="mt-2" />
                      </div>

                      
                        <div>
                          <Label for_id="item_description" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">{{ $t('description') }}</Label>
                          <Textarea rows="8" id="item_description" v-model="communityItemForm.description"></Textarea>
                          <InputError :message="communityItemForm.errors.description" class="mt-2" />
                        </div>
                        <div>
                          <Label  
                            for_id="image_path"
                            ...
                            >{{ $t("image") }}</Label
                          >
                          <FileUpload v-model="communityItemForm.image_path" target_input="image_path" :selectedFile="selectedImage" @clear="selectedImage = null"/>
                          <InputError :message="communityItemForm.errors.image_path" class="mt-2" />            
                        </div>
                    ...
                    h(
                      "div",
                      {
                        ...
                        onClick: (e) => {
                          ...
                          communityItemForm.link = row.link
                          communityItemForm.description = row.description ? row.description : ""
...
const communityItemsFormSubmit = () => {
  if(communityItemForm.id) {
    communityItemForm.post(route('frontend.page.communityItems.update', communityItemForm), {
      onSuccess: () => {
