<template>
  <LayoutTab>
    ...
    <div class="">
      ...
      <div class="relative">
        <table id="slider" class="table w-full text-sm text-left rtl:text-right">
          <thead class="text-center bg-white dark:bg-boxdark">
            <tr>
              ...
              <th class="w-[5%]">{{ $t("image") }} (Desktop)</th>
              <th class="w-[5%]">{{ $t("image") }} (Mobile)</th>
              <th class="w-[25%]">{{ $t("title") }}</th>
              <th class="w-[30%]">{{ $t("link") }}</th>
...
onMounted(() => {
  tableData.value = $("#slider").DataTable({
    ...
    ajax: {
      url: route("frontend.page.sliders.list"),
      type: "GET",
    },
    columns: [
      {
      ...
      { data: "title", name: "title" },
      {
        ...
        render: function (data, type, row, meta) {
          if (data) {
            return `<a target="blank_" class="text-purple-700" title="Click for open" href="${data}">${data}</a>`;
          }
          return "";
        },
      },
