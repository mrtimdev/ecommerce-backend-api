<template>
  <LayoutTab>
    <Head :title="$t('frontend.taxinfos')" />
    <div class="">
      <div
      ...
      >
        ...
        <div>You have limited of Tax Items.</div>
      </div>
      <div
        v-else
        class="flex items-center p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300"
      ...
      >
        ...
        <span class="sr-only">Info</span>
        <div>NOTE: You have only 5 of Tax Items.</div>
      </div>
      <div class="relative">
        <div class="mt-4">
          <div class="grid gap-6 mb-6 md:grid-cols-2">
            ...
            >
              ...
              <div>
                <div class="">
                  <form @submit.prevent="taxInfoFormSubmit" enctype="multipart/form-data">
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                      <div>
                        <Label
                          for_id="label"
                        ...
                        <input
                          ...
                          :placeholder="$t('label')"
                        />
                        <InputError :message="taxInfoForm.errors.label" class="mt-2" />
                      </div>
                      <div>
                        <Label
                          for_id="title"
                        ...
                        <input
                          ...
                          id="title"
                          ref="title"
                          v-model="taxInfoForm.title"
                          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-boxdark-1 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 dark:bg-tim-11"
                          :placeholder="$t('title')"
                        />
                        <InputError :message="taxInfoForm.errors.title" class="mt-2" />
                      </div>
                    </div>
            ...
            >
              ...
              <div>
                <form
                  @submit.prevent="taxInfoItemsFormSubmit"
                ...
                >
                  <div class="grid gap-6 mb-6 md:grid-cols-2">
                    <div>
                      ...
                      <input
                        ...
                        id="item_title"
                        ref="item_title"
                        v-model="taxInfoItemForm.title"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-boxdark-1 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500 dark:bg-tim-11"
                        :placeholder="$t('title')"
                      />
                      <InputError :message="taxInfoItemForm.errors.title" class="mt-2" />
                    </div>
                    <div>
                      <Label
                        for_id="pdf"
                      ...
                      <InputError :message="taxInfoItemForm.errors.pdf" class="mt-2" />
                    </div>
                  </div>
                  ...
                  <div class="flex justify-center gap-5 items-center">
                    ...
                    <button
                      :disabled="is_limited ? true : false"
                      :class="{ 'cursor-not-allowed': is_limited }"
                      class="w-full focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-2 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900"
                    >
                      {{ $t(buttonSubmitItem) }}
                    </button>
...
const taxInfoStatus = useForm({
  id: null,
  is_active: undefined,
});
const taxInfoForm = useForm({
  id: props.taxInfo ? props.taxInfo.id : null,
  label: props.taxInfo ? props.taxInfo.label : "",
  title: props.taxInfo ? props.taxInfo.title : "",
});

const taxInfoItemForm = useForm({
  id: null,
  title: "",
  pdf: null,
  tax_info_id: props.taxInfo.id ? props.taxInfo.id : null,
});

const taxInfoFormSubmit = () => {
  taxInfoForm.post(route("frontend.page.taxInfos.store"), {
    onSuccess: () => {
...
onBeforeUnmount(() => {
  taxInfoItem.value = false;
});
onMounted(() => {
  tableData.value = $("#taxInfoItems").DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [[1, "desc"]],
    ...
    ajax: {
      url: route("frontend.page.taxInfoItems.list"),
      type: "GET",
    },
    columns: [
      {
      ...
      { data: "title", name: "title" },
      {
        ...
        render: function (data, type, row, meta) {
          if (!data) return "";
          return `<a href="${route("frontend.page.taxInfoItems.pdf-preview", {
            taxInfoItem: row.id,
                    ...
                    default: () => [
                      h(
                        "div",
                        {
                          ...
                          onClick: (e) => {
                            ...
                            taxInfoItemForm.id = row.id;
                            taxInfoItemForm.title = row.title;
                            taxInfoItemForm.pdf = row.pdf;
                            taxInfoItemForm.clearErrors();
                      ...
                      h(
                        "div",
                        {
                          ...
                          onClick: (e) => {
                            e.preventDefault();
                            events.emit("confirm:taxinfo-active", row);
                          },
                        },
                        [
...
const cancelUpdateItem = () => {
  ...
  taxInfoItemForm.id = null;
  taxInfoItemForm.title = "";
  taxInfoItemForm.pdf = null;
  taxInfoItemForm.clearErrors();
  item_title.value.focus();
...
const handleFileUpload = (event) => {
  ...
  if (selectedFile) {
    if (selectedFile.type === "application/pdf") {
      taxInfoItemForm.pdf = selectedFile;
      errorMessage.value = "";
...
const taxInfoItemsFormSubmit = () => {
  errorMessage.value = "";
  if (taxInfoItemForm.id) {
    taxInfoItemForm.post(route("frontend.page.taxInfoItems.update", taxInfoItemForm), {
      onSuccess: () => {
        taxInfoItemForm.reset();
        item_title.value.focus();
        ...
        nextTick(() => {
          ...
          Toast.fire({
            icon: "success",
            html: `<b>Success:</b> Tax Info Item updated.`,
          });
        });
      },
      onError: () => {},
    ...
    taxInfoItemForm.post(route("frontend.page.taxInfoItems.store"), {
      onSuccess: () => {
        taxInfoItemForm.reset();
        item_title.value.focus();
        tableData.value.ajax.reload();
        ...
        nextTick(() => {
          ...
          if (!is_limited) {
            ...
            Toast.fire({
              icon: "success",
              html: `<b>Success:</b> Tax Info Item created.`,
            });
          }
        });
      },
...
events.on("confirm:confirmed", (data) => {
  ...
  var routeName = route("frontend.page.taxInfoItems.destroy.selected", {
    ids: items,
  });
  taxInfoItemForm.post(routeName, {
    preserveScroll: true,
    onSuccess: () => {
      ...
      Toast.fire({
        icon: "success",
        html: `<b>Success:</b> Tax Info Item deleted.`,
      });
      if (taxInfoItemForm.id) {
        cancelUpdateItem();
      }
...
events.on("confirm:taxinfo-active", (item) => {
  proxy.$swal
    .fire({
    ...
    .then((result) => {
      if (result.isConfirmed) {
        taxInfoStatus.id = item.id;
        taxInfoStatus.is_active = item.is_active ? false : true;
        taxInfoStatus.post(
          route("frontend.page.taxInfoItems.change-status", taxInfoStatus),
          {
            onSuccess: () => {
              const Toast = proxy.$swal.mixin({
                toast: true,
