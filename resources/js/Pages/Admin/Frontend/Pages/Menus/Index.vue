<template>
  <LayoutTab>
    ...
    <div class="">
      ...
      >
        <div class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          <h2 class="text-md font-medium text-gray-700 hover:text-purple-600 dark:text-gray-400 dark:hover:text-white">{{ $t('menus') }}</h2>
        </div>
        ...
        <table id="menu"
          ...
          <thead class="text-center bg-white dark:bg-boxdark">
            <tr>
              ...
              <th class="w-[5%]">{{ $t('name') }}</th>
              <th class="w-[10%]">{{ $t('brands') }}</th>
              <th class="w-[10%]">{{ $t('models') }}</th>
              <th class="w-[10%]">{{ $t('categories') }}</th>
              <th class="w-[10%]">{{ $t('fuel_types') }}</th>
              <th class="w-[10%]">{{ $t('steerings') }}</th>
              <th class="w-[10%]">{{ $t('drive_types') }}</th>
              <th class="w-[10%]">{{ $t('passengers') }}</th>
              <th class="w-[10%]">{{ $t('locations') }}</th>
    ...
    <AddOrEdit :show-modal="false" :brands="brands" :categories="categories" :fuel_types="fuel_types" :steerings="steerings" :locations="locations" :drive_types="drive_types" :passengers="passengers"/>
  </LayoutTab>
</template>
...
onMounted(() => {
  ...
  tableData.value = $('#menu').DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [[1, 'desc']],
    ...
    ajax: {
      url: route('frontend.page.menus.list'),
      type: 'GET',
    },
    columns: [
      {
      ...
      },
      {
        data: "code", name: "code",},
      { data: 'name', name: 'name' },
      { data: 'brands_name', name: 'brands_name' },
      { data: 'models_name'},
      { data: 'categories_name'},
      { data: 'fuel_types_name'},
      { data: 'steerings_name'},
      { data: 'drive_types_name'},
      { data: 'passengers_name'},
      { data: 'locations_name'},
      {
        data: "action",
        orderable: false,
