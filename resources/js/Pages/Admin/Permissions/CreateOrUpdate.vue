...
const page_title = ref(t("permission.create"));
const button_name = ref(t("save"));
const name = ref(null);
const form = useForm({
  id: props.permission ? (props.permission.id ? props.permission.id : null) : null,
  name: props.permission ? (props.permission.name ? props.permission.name : "") : "",
  display_name: props.permission
...
const submitForm = () => {
  ...
  if (props.permission.id) {
    form.post(route("permissions.update", form), {
      preserveScroll: true,
      onSuccess: () => {
        events.emit("toaster", {
          type: "success",
          action: "create",
          message: `${t("permission")} [${form.name}] ${t("successfully_updated")}`,
        });
        form.reset();
      },
    });
    ...
    form.post(route("permissions.store"), {
      ...
      onSuccess: () => {
        events.emit("toaster", {
          type: "success",
          action: "create",
          message: `${t("permission")} [${form.name}] ${t("successfully_added")}`,
        });
        form.reset();
      },
    });
...
onMounted(() => {
  nextTick(() => {
    name.value.focus();
    if (props.permission.id) {
      page_title.value = t("permission.update");
