              ...
              <div class="col-span-12 md:col-span-6">
                ...
                <div class="relative">
                  <select
                    ...
                    :class="{ 'opacity-50 cursor-not-allowed': !clientPackages.length }"
                  >
                    <option value="" disabled selected>{{ $t("select_package") }}</option>
                    <option v-for="pkg in clientPackages" :key="pkg.id" :value="pkg.id">
                      #{{ pkg.reference_no }} — {{ pkg.date }}
                    </option>
                  </select>
                  <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                    <i class="fi fi-rr-box text-gray-400"></i>
                  </div>
                </div>
                <p v-if="!clientPackages.length && form.client_id" class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                  {{ $t('no_packages_found') }}
                </p>
              </div>
...
const loadPackageItems = async () => {
    try {
        ...
        packageItems.value = response.data.items.map(item => () => {
            return {
                id: item.id,
        ...
        // Uncomment the following lines if you want to use the response structure directly
...
const loadPackageItems_ = () => {
  ...
  const selectedPackage = clientPackages.value.find(pkg => pkg.id === parseInt(form.package_id));

  if (selectedPackage && selectedPackage.items) {
    packageItems.value = selectedPackage.items.map((item) => ({
      id: item.id,
      product_name: item.product?.name || 'N/A',
      quantity: item.quantity,
      unit_name: item.unit?.name || 'N/A',
