<template>
  <LayoutTab>
    ...
    <div class="">
      ...
      >
        <div class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          <h2 class="text-md font-medium text-gray-700 hover:text-purple-600 dark:text-gray-400 dark:hover:text-white">{{ $t('options') }}</h2>
        </div>
        ...
        <table id="option"
          ...
          <thead class="text-center bg-white dark:bg-boxdark">
            <tr>
              ...
              <th class="w-[47%]">{{ $t('name') }}</th>
              <th class="w-[40%]">{{ $t('group_name') }}</th>
    ...
    <AddOrEdit :show-modal="false" :optionGroups="optionGroups"/>
  </LayoutTab>
</template>
...
onMounted(() => {
  ...
  tableData.value = $('#option').DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [[1, 'desc']],
