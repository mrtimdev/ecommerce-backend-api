<template>
  <LayoutTab>
    <Head :title="$t('frontend.optionGroups')"/>
    <div class="">
      <div
      ...
      >
        <div class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          <h2 class="text-md font-medium text-gray-700 hover:text-purple-600 dark:text-gray-400 dark:hover:text-white">{{ $t('optionGroups') }}</h2>
        </div>
...
onMounted(() => {
  ...
  tableData.value = $('#optionGroup').DataTable({
    processing: true,
    serverSide: true,
    pageLength: 10,
    order: [[1, 'desc']],
