            ...
            <form @submit.prevent="colorStore.submitForm">
              <div class="p-4 md:p-5">
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                  <div>
                    <label for_id="code" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">{{
                      $t('color.code') }}</label>
                    
                      <ColorPicker 
                        alpha-channel="hide"
                        :color="colorStore.form.code"
                        :visible-formats="['hex', 'rgb']"
                        @color-change="handleColorChange"/>
                    <InputError :message=" colorStore.form.errors.code " class="mt-2" />
                  </div>
                  <div>
                    <label for_id="name" class="block mb-2 text-sm font-medium text-gray-700 dark:text-white">{{
                      $t('name') }}</label>
                    <input type="text" ref="name" id="name" v-model=" colorStore.form.name "
                ...
                <div class="mb-6">
                  <Label for_id="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ $t('description') }}</Label>
...
<script setup>
  ...
  const handleColorChange = (e) => {
    colorStore.form.code = e.cssColor
    console.log({e})
  }
